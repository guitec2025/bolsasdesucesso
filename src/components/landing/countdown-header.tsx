...
export function CountdownHeader() {
  ...
  useEffect(() => {
    // Set target date to May 11th of the current or next year
    const now = new Date();
    let targetYear = now.getFullYear();
    const targetDateForCurrentYear = new Date(targetYear, 4, 11); // Month is 0-indexed (4 = May), Day is 11

    if (now > targetDateForCurrentYear) {
      targetYear += 1;
    }
    const targetDate = new Date(targetYear, 4, 11);
