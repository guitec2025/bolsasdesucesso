...
export function CountdownHeader() {
  ...
  useEffect(() => {
    // Set target date to April 13th of the current or next year
    const now = new Date();
    let targetYear = now.getFullYear();
    const targetDateForCurrentYear = new Date(targetYear, 3, 13); // Month is 0-indexed (3 = April), Day is 13

    if (now > targetDateForCurrentYear) {
      targetYear += 1;
    }
    const targetDate = new Date(targetYear, 3, 13);
