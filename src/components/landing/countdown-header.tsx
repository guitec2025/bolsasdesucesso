...
export function CountdownHeader() {
  ...
  useEffect(() => {
    // Set target date to March 21st of the current or next year
    const now = new Date();
    let targetYear = now.getFullYear();
    const targetDateForCurrentYear = new Date(targetYear, 2, 21); // Month is 0-indexed (2 = March), Day is now 21

    if (now > targetDateForCurrentYear) {
      targetYear += 1;
    }
    const targetDate = new Date(targetYear, 2, 21);
